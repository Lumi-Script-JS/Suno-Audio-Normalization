...
`;
  ...
  const processAudio = async () => {
    ...
    try {
      ...
      if (lockLra && meas_LRA) {
         LRA_RUN = Math.round(parseFloat(meas_LRA)).toString();
      }
      ...
      await ffmpeg.exec([
        '-y', '-hide_banner', '-i', outName,
        '-af', `loudnorm=I=${parseFloat(targetLufs)}:TP=${parseFloat(targetTp)}:LRA=${LRA_RUN}:print_format=json`,
        '-f', 'null', '-'
      ]);
      ...
      if (finalMatch) {
         try {
           ...
           const nType = fMeas.normalization_type || (EFFECTIVE_LINEAR === 'true' ? 'Linear' : 'Dynamic');
           let dynWarn = '';
           if (nType.toLowerCase() === 'dynamic') {
             dynWarn = '\n\n*Note: Dynamic normalization was used. LRA is impacted to prevent True Peak clipping.*';
           }
...
Output Integrated:   ${fMeas.input_i || '0.0'} LUFS
Output True Peak:     ${fMeas.input_tp || '0.0'} dBTP
Output LRA:            ${fMeas.input_lra || '0.0'} LU
Output Threshold:    ${fMeas.input_thresh || 'N/A'} LUFS
Normalization Type:   ${nType}
Target Offset:        ${fMeas.target_offset || '0.0'} LU${dynWarn}`);
         } catch (e) {
